...
function initTimestampInputs() {
    ...
    // Part 4 has 10 groups of 3 questions each (71-73, 74-76, ..., 98-100)
    for (let i = 1; i <= 10; i++) {
        const startQ = 71 + (i - 1) * 3;
        const endQ = startQ + 2;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    autoSplitBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('/api/auto-split-part4', {
                method: 'POST',
                body: formData
            });
    ...
    splitBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('/api/split-part4', {
                method: 'POST',
                body: formData
            });
